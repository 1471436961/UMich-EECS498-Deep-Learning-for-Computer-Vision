...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = torch.max(torch.zeros(N, W1.shape[1], device='cuda'), X.mm(W1) + b1)
...
def nn_forward_backward(
    ...
    grads['W2'] = h1.t().mm(dscores) / N + 2 * reg * W2
    grads['b2'] = torch.sum(dscores, dim=0) / N
    
    dh1 = dscores.mm(W2.t()) / N
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    h1 = torch.max(torch.zeros(X.shape[0], params['W1'].shape[1], device='cuda'), X.mm(params['W1']) + params['b1'])
    y_pred = torch.argmax(h1.mm(params['W2']) + params['b2'], dim=1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
...
def nn_get_search_params():
    ...
    learning_rate_decays = [0.96, 0.95] 
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
        ...
        for hz in hidden_sizes:
            for reg in regularization_strengths:
                for lrd in learning_rate_decays:
                    print(f'train with lr:{lr}, hz:{hz}, reg:{reg}, lrd:{lrd}.') 
                    net = TwoLayerNet(3 * 32 * 32, hz, 10, dtype=data_dict['X_train'].dtype, device=data_dict['X_train'].device)
                    stat = net.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'], 
                                      num_iters=1000, batch_size=1000, learning_rate=lr, learning_rate_decay=lrd, reg=reg, verbose=True)
