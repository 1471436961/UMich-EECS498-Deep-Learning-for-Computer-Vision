    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            ...
            if margin > 0:
                ...
                # ∂(WTX) / ∂W = X
                # 更新错误类别的梯度(降低错误类别的分数, 增大dW，更新权重时W会减少)
                dW[:, j] += X[i]
                # 更新正确类别的梯度(增加正确类别的分数, 减小dW，更新权重时W会增加)
                dW[:, y[i]] -= X[i]
                #######################################################################
                #                       END OF YOUR CODE                              #
                #######################################################################
...
def svm_loss_vectorized(
    ...
    num_train = X.shape[0] 
    scores = X.mm(W) 
    correct_scores = scores[torch.arange(num_train), y].reshape(-1, 1) 
    margins = scores - correct_scores + 1  
    margins[torch.arange(num_train), y] = 0  # 将正确类别的间隔置为 0
    ####################################NOTES####################################
    # torch.clamp(input, min, max, out=None) -> Tensor, 
    # 将输入张量中的每个元素裁剪到 [min, max] 区间内。
    # 如果元素小于 min，则将其设置为 min；大于 max，则将其设置为 max；否则保持不变。
    #     input: 输入张量。
...
def sample_batch(
    ...
    # 用于生成一个随机整数张量，其中的元素是从均匀分布中随机采样的整数。
    # 它的作用类似于 NumPy 中的 numpy.random.randint()。
    # 参数说明：
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    # torch.argmax(input, dim=None, keepdim=False) 用于返回张量中最大值所在的索引。
