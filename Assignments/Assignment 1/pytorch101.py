...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    multiples = [x for x in torch.arange(start, stop) if x % 10 == 0]
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    '''
    torch.repeat_interleave(input, repeats, dim=None) -> Tensor, 用于重复张量中的元素。它可以沿着指定的维度重复张量的元素。
        input: 输入张量。
        repeats: 每个元素的重复次数。可以是一个整数（所有元素重复相同次数）或一个张量（每个元素重复指定次数）。
    ...
    '''
    torch.scatter_add_(dim, index, src) -> Tensor, 用于将源张量的值按照索引累加到目标张量中。它是一个原地操作（in-place），会直接修改目标张量。
        dim: 沿着哪个维度进行累加。
...
实现思路
排序：
    对输入张量 x 进行排序，得到一个排序后的张量 sorted_x 和对应的索引 sorted_indices。
    排序后，相同的值会相邻。
找到唯一值：
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    # 计算相邻元素的差值，找到唯一值的分界点（使用 torch.cat 在差值前面补一个 1，确保第一个元素被视为唯一值）
