...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # dists[i, j] = ||x_train[i] - x_test[j]||^2 = ||x_train[i]||^2 + ||x_test[j]||^2 - 2 * x_train[i] @ x_test[j]
    train_norm = torch.sum(x_train_flat ** 2, dim=1, keepdim=True)  # 形状: (num_train, 1)
    test_norm = torch.sum(x_test_flat ** 2, dim=1)                  # 形状: (num_test,)
    dot_product = torch.mm(x_train_flat, x_test_flat.t())           # 形状: (num_train, num_test)

    dists = train_norm + test_norm - 2 * dot_product  # 形状: (num_train, num_test)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels_me(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    '''
    torch.topk(input, k, dim=None, largest=True, sorted=True), 用于返回张量中沿着指定维度的前k个最值及其对应的索引。
    参数说明：
        input (Tensor): 输入张量。
        k (int): 需要返回的最大值（或最小值）的数量。
        dim (int, optional): 沿着哪个维度进行操作。如果未指定，则默认对输入张量的最后一个维度进行操作。
        largest (bool, optional): 如果为 True，返回前k个最大值；如果为 False，返回前k个最小值。默认为 True。
        sorted (bool, optional): 如果为 True，返回的结果会按照顺序排列（从大到小或从小到大）。默认为 True。
    '''
...
def predict_labels_deepseek(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        ...
        # 统计每个标签的出现次数
        '''
        torch.bincount(input, weights=None, minlength=0), 用于统计非负整数张量中每个值的出现次数。是一个非常高效的函数(O(n))，特别适合处理分类任务中的标签统计问题。
        参数说明：
            input (Tensor): 输入张量，必须是一维的且包含非负整数。
            weights (Tensor, optional): 可选参数，与 input 形状相同的权重张量。如果提供，bincount() 会返回加权和而不是简单的计数。
            minlength (int, optional): 输出张量的最小长度。如果 input 中的最大值加 1 小于 minlength，则输出张量会被填充到 minlength 的长度。
        返回值：
            output (Tensor): 一个一维张量，表示每个非负整数的出现次数（或加权和）。张量的长度为 max(input) + 1 或 minlength，取两者中的较大值。
        '''
        counts = torch.bincount(labels)

        # 找到票数最多的标签
        '''
        torch.where(condition, x, y), 用于根据条件从两个张量中选择元素。可以根据条件返回满足条件的元素的位置，或者从两个张量中选择元素。
        参数说明：
            condition (Tensor): 条件张量，布尔类型（True 或 False）。
            x (Tensor): 当 condition 为 True 时选择的元素。
            y (Tensor): 当 condition 为 False 时选择的元素。
        返回值：
            output (Tensor): 一个张量，形状与 condition、x 和 y 广播后的形状相同。output 中的每个元素根据 condition 的值从 x 或 y 中选择。
        '''
...
def knn_cross_validate(
    ...
    # Replace "pass" statement with your code
    '''
    torch.chunk(input, chunks, dim=0), 用于将张量沿着指定维度分割成多个块（chunks）。每个块的大小尽可能相等，但如果张量的大小不能被块数整除，最后一个块可能会较小。
    参数说明：
        input (Tensor): 输入张量。
