...
def prepocess_input_sequence(
    ...
    for number in numbers:
        if number in spc_tokens:
            out.append(token_dict[number])
        else:
            digits = list(number)
            out.extend([token_dict[digit] for digit in digits])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_single(
    ...
    similarity =  torch.zeros(K, K, dtype=torch.float32, device='cpu')
    attention_weight = torch.zeros(K, K, dtype=torch.float32, device='cpu')
    out = torch.zeros(K, M, dtype=torch.float32, device='cpu')
    
    for i in range(K):
        for j in range(K):
            similarity[i][j] = query[i].dot(key[j]) / math.sqrt(M)
        
        attention_weight[i] = torch.softmax(similarity[i], dim=-1)
        out[i] = value.t().mv(attention_weight[i])    
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    similarity = torch.zeros(N, K, K, dtype=torch.float32, device='cpu')
    attention_weight = torch.zeros(N, K, K, dtype=torch.float32, device='cpu')
    out = torch.zeros(N, K, M, dtype=torch.float32, device='cpu')

    for i in range(K):
        for j in range(K):
            similarity[:, i, j] = torch.sum(query[:, i, :] * key[:, j, :], dim=-1) / math.sqrt(M)

        attention_weight[:, i, :] = torch.softmax(similarity[:, i], dim=-1)
        out[:, i, :] = value.permute(0, 2, 1).bmm(attention_weight[:, i, :].view(N, K, 1)).view(N, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    similarity = query.bmm(key.permute(0, 2, 1)) / math.sqrt(M)
    ...
    if mask is not None:
        ...
        # Replace "pass" statement with your code
        #################################NOTES####################################
        # torch.masked_fill(input, mask, value, *, out=None)
        #                           与相关函数的对比
        #       函数	                作用	    是否修改原张量	  典型场景
        # torch.masked_fill()	   按掩码替换指定值	    ❌	    数据清洗、注意力掩码
        # torch.where()	       按掩码从两个张量中选择元素	❌	         条件选择
        # masked_select()	   按掩码提取元素为一维张量	❌	         数据筛选
        # index_fill()	          按索引维度填充值	    ❌	       特定维度填充
        #################################NOTES####################################
        similarity = torch.masked_fill(similarity, mask, -1e9)
    # Replace "pass" statement with your code
    weights_softmax = torch.softmax(similarity, dim=-1)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        c_q_k = math.sqrt(6 / (dim_in + dim_q))
        c_v = math.sqrt(6 / (dim_in + dim_v))
        torch.nn.init.uniform_(self.q.weight, -c_q_k, c_q_k)
        torch.nn.init.uniform_(self.k.weight, -c_q_k, c_q_k)
        torch.nn.init.uniform_(self.v.weight, -c_v, c_v)
        ###################################NOTES##################################
        # 在 PyTorch 中，可以通过以下方法自定义 torch.nn.Linear 层的权重初始化：
        # 1. 直接修改权重张量 
        #     方法1：使用 nn.init工具函数
        #     方法2：手动赋值, 直接操作 weight.data 或 bias.data：
        # 2. 通过 reset_parameters()方法
        #     自定义初始化逻辑可封装到 reset_parameters()中
        # 3. 初始化整个模型
        #     对所有Linear层应用统一初始化, model.apply(init_weights)  # 递归应用初始化函数
        # 4. 特殊初始化场景
        #     (1)稀疏初始化 (2)分块差异化初始化
        # 选择依据：
        #     简单需求：直接使用 nn.init 函数
        #     复杂逻辑：操作 .data 或继承子类
        #     工程化：通过 apply() 批量初始化
        ###################################NOTES##################################
        ##########################################################################
    ...
    def forward(
        ...
        y, self.weights_softmax = scaled_dot_product_no_loop_batch(query, key, 
                                                                   value, mask)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        # Replace "pass" statement with your code
        ################################NOTES####################################
        # torch.nn.ModuleList 是 PyTorch 中用于动态存储子模块（如神经网络层）的容器，
        # 与Python原生列表不同，它能正确注册子模块的参数，确保这些参数可被优化器识别和训练。
        # 1.核心特性
        #     参数注册：自动将子模块的参数添加到父模块的 parameters() 中。
        #     动态性：支持类似列表的操作（追加、索引、迭代等），但不能直接切片。
        #     非计算层：仅作为容器，没有 forward() 方法。
        # 2.与普通列表的区别
        #     特性	     nn.ModuleList	             Python list
        #     参数注册	✅ 自动注册子模块参数	    ❌ 参数不可见
        #     切片操作	❌ 不支持 model_list[1:3]	✅ 支持
        #     序列化支持	✅ 可被 torch.save正确保存	❌ 无法保存参数
        #     适用场景	动态构建神经网络层	        存储非模块对象（如超参数）
        # 3. 典型应用场景
        #     (1)动态构建网络 (2)多分支结构 (3)共享权重模块管理
        # 4. 进阶技巧
        #     (1)自定义模块的延迟初始化 (2)与 nn.ModuleDict 配合使用
        ################################NOTES####################################
        self.heads = torch.nn.ModuleList([
            SelfAttention(dim_in, dim_out, dim_out) for _ in range(num_heads)
        ])
        self.recover = torch.nn.Linear(dim_out * num_heads, dim_in)

        c = math.sqrt(6 / (dim_out * num_heads + dim_in))
        torch.nn.init.uniform_(self.recover.weight, -c, c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        y = self.recover(torch.cat([
            self_attention(query, key, value, mask) for self_attention in self.heads
        ], dim=2))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        # 为什么必须用 nn.Parameter？
        #     参数注册机制：
        #         PyTorch通过nn.Module的__setattr__方法自动检测nn.Parameter，
        #         将其加入参数列表。普通张量会被忽略。
        #     优化器依赖：
        #         优化器的param_groups仅包含nn.Parameter或通过parameters()方法返回的张量。
        #     模型保存：
        #         torch.save(model.state_dict())只会保存nn.Parameter和缓冲区（register_buffer）
        # 
        # 冻结部分参数: model.scale.requires_grad_(False)  # 固定scale，只训练shift
        # 
        # 与 register_buffer 的区别
        #     nn.Parameter：需要梯度，用于可训练参数。
        #     register_buffer：无需梯度，用于持久化但不训练的统计量（如BatchNorm的running_mean）。
        #################################NOTES####################################
    ...
    def forward(self, x: Tensor):
        ...
        # 统计量计算维度	  沿特征维度（最后一维）	      沿批量维度（第一维）
        #   适用场景	    RNN/Transformer等变长序列	      CNN等固定输入
        #   推理行为	      无需区分训练/测试模式	     测试时使用移动平均统计量
        #   参数数量	    与特征维度相同（emb_dim）	   与通道数相同（num_features)
        # 
        # 为何不用torch.std？
        #     PyTorch的var()已足够计算标准差（std = sqrt(var)），且更高效。
        # unbiased=False的意义：
        #    使用总体方差（除以n而非n-1），与原始论文保持一致。
        #################################NOTES####################################
        mean = x.mean(dim=-1, keepdim=True) # 保持维度以便广播
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.linear_in_to_hid = torch.nn.Linear(inp_dim, hidden_dim_feedforward)
        self.linear_hid_to_out = torch.nn.Linear(hidden_dim_feedforward, inp_dim)

        c = math.sqrt(6 / (inp_dim + hidden_dim_feedforward))
        torch.nn.init.uniform_(self.linear_in_to_hid.weight, -c, c)
        torch.nn.init.uniform_(self.linear_hid_to_out.weight, -c, c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(self, x):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        h = torch.relu(self.linear_in_to_hid(x))
        y = self.linear_hid_to_out(h)
        #################################NOTES####################################
        #                 torch.relu()和torch.nn.ReLU()的区别
        #                           基本定义
        # 特性	                torch.relu()	                  torch.nn.ReLU()
        # 类型	            函数式接口（Function）	              类模块（Module）
        # 调用方式	       直接调用（如 torch.relu(x)）          需要实例化（如 nn.ReLU()(x)）
        # 是否可注册为网络层	❌不能作为网络层添加到 nn.Sequential	 ✅ 可以
        # 原地操作支持	   ✅有 torch.relu_() 的原地操作版本	     ❌无原地操作版本
        # 
        #                          核心区别
        # 方面	                  torch.relu()	             torch.nn.ReLU()
        # 功能实现	             纯函数，无状态	      类实例，可包含状态（如训练/测试模式）
        # 序列化支持	            ❌不能单独保存	          ✅可随模型保存和加载
        # 参数管理	              ❌无参数	           ✅虽无参数，但仍属于 nn.Module
        # 性能	               稍快（直接C++实现）	    稍慢（有Python层调用开销）
        #
        # 如何选择？
        #     选择 torch.relu() 当：       
        #         需要快速临时计算（如调试时）        
        ...
        #         在自定义 forward() 中直接使用        
        #    选择 torch.nn.ReLU() 当：        
        #         构建 nn.Sequential 或 nn.Module 子类        
        #         需要与其他网络层统一管理        
        #         可能扩展为 nn.ReLU6 或 nn.LeakyReLU
        #################################NOTES####################################
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.MultiHeadBlock = MultiHeadAttention(num_heads, emb_dim, 
                                                 emb_dim // num_heads)
    ...
    def forward(self, x):
        ...
        out1_norm = self.layernorm1(out1 + x)

        out2 = self.dropout(out1_norm)

        out3 = self.feedforward(out2)
        out3_norm = self.layernorm2(out3 + out2)
...
def get_subsequent_mask(seq):
    ...
    # torch.triu(input, diagonal=0, *, out=None)构造上三角矩阵，diagonal为对角线偏移量
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        # Replace "pass" statement with your code
        self.attention_self = MultiHeadAttention(num_heads, emb_dim, 
                                                 emb_dim // num_heads)
        self.attention_cross = MultiHeadAttention(num_heads, emb_dim, 
                                                  emb_dim // num_heads)
    ...
    def forward(
        ...
        out1_norm = self.norm1(out1 + dec_inp)

        out2 = self.dropout(out1_norm)

        out3 = self.attention_cross(out2, enc_inp, enc_inp)
        out3_norm = self.norm2(out3 + out2)

        out4 = self.dropout(out3_norm)

        out5 = self.feed_forward(out4)
        out5_norm = self.norm3(out5 + out4)

        y = self.dropout(out5_norm)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    y = positions.unsqueeze(1).repeat(1, M)

    y = y.unsqueeze(0)
    ###################################NOTES######################################
    # torch.unsqueeze(input, dim), dim：要插入新维度的位置（从0开始计数），支持负数索引
    # 返回一个新张量，维度数比输入多1，新维度的长度为1。
    # 
    # 与 squeeze() 的关系：两者互为逆操作。
    #     unsqueeze(dim)：在指定位置插入长度为1的维度。
    #     squeeze(dim)：移除指定位置长度为1的维度（若长度不为1则无效）。
    # 
    # 无内存复制：与 reshape 或 view 不同，unsqueeze() 返回的张量与原张量共享存储。
    ###################################NOTES######################################
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    div_term1 = torch.exp(torch.arange(0, M, 2, dtype=torch.float32) * (-math.log(10000.0)) / M)
    div_term2 = torch.exp(torch.arange(1, M, 2, dtype=torch.float32) * (-math.log(10000.0)) / M)
...
class Transformer(nn.Module):
    def __init__(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.emb_layer = torch.nn.Embedding(vocab_len, emb_dim)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
