...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    #     target,  # 真实标签（[N](类别索引（常用）)或[N, C](类别概率（如标签平滑后）)）
    #     weight=None,    # 各类别的权重（Tensor，可选）, 处理类别不平衡
    #     ignore_index=-100,  # 忽略的标签索引（默认-100）
    #     reduction='mean',   # 损失聚合方式：'mean'|'sum'|'none'
    #     label_smoothing=0.0  # 标签平滑系数，范围[0, 1), label_smoothing=0.1
    #                            表示将真实标签从1.0平滑为0.9，其余类别共享0.1/(C-1)
    # )
    # cross_entropy 不直接支持多标签分类（每个样本可能属于多个类别），此时应使用 
    # torch.nn.BCEWithLogitsLoss
    # 性能优化建议
    #     启用CuDNN：确保使用GPU时启用加速：torch.backends.cudnn.enabled = True
    #     避免重复计算：如果同时需要损失和预测概率，直接使用 logits 计算：
    #                  probs = F.softmax(logits, dim=1)  # 无需重复计算
    ################################NOTES#####################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        self.linear_img_to_hid = torch.nn.Linear(self.image_encoder.out_channels * 4 * 4, 
                                                 hidden_dim).to(device='cuda', dtype=torch.float64)
        self.word_embedding = WordEmbedding(vocab_size, 
                                            embed_size=wordvec_dim).to(device='cuda', 
                                                                       dtype=torch.float64)
        ...
        if self.cell_type == 'rnn':
            ...
            self.conv_attention = torch.nn.Conv2d(self.image_encoder.out_channels, hidden_dim, 
                                                  kernel_size=1).to(device='cuda', 
                                                                    dtype=torch.float64)
            self.AttentionLSTM = AttentionLSTM(wordvec_dim, hidden_dim).to(device='cuda', 
                                                                           dtype=torch.float64)

        self.linear_hid_to_score = torch.nn.Linear(hidden_dim, vocab_size).to(device='cuda',
                                                                              dtype=torch.float64)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images, captions):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        image_features = self.image_encoder(images).to(device='cuda', 
                                                       dtype=torch.float64)
        flatten_features = image_features.view(image_features.shape[0], -1)
        h0 = self.linear_img_to_hid.to(device='cuda', 
                                       dtype=torch.float64)(flatten_features)

        word_embedding = self.word_embedding(captions_in)
        
        if self.cell_type == 'rnn':
            hidden_states = self.RNN(word_embedding, h0).to(device='cuda', 
                                                            dtype=torch.float64)
        elif self.cell_type == 'lstm':
            hidden_states = self.LSTM(word_embedding, h0).to(device='cuda', 
                                                             dtype=torch.float64)
        else:
            attention_matrix = self.conv_attention(image_features)
            hidden_states = self.AttentionLSTM(word_embedding, attention_matrix)
            
        scores = self.linear_hid_to_score.to(device='cuda', 
                                             dtype=torch.float64)(hidden_states)
        loss = temporal_softmax_loss(scores, captions_out.to(device='cuda'), 
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == 'lstm':
            prev_c = torch.zeros(N, hidden_state.shape[1], 
        ...
        word_embedding = self.word_embedding([self._start for _ in range(N)])
 
        for i in range(max_length):
            if self.cell_type == 'rnn':
                hidden_state = self.RNN.step_forward(word_embedding, hidden_state)
            elif self.cell_type == 'lstm':
                hidden_state, prev_c = self.LSTM.step_forward(word_embedding, 
                                                              hidden_state, prev_c)
            else:
                attn, attn_weights = dot_product_attention(prev_h, images)
                attn_weights_all[:,t] = attn_weights
                hidden_state, prev_c = self.AttentionLSTM.step_forward(word_embedding, 
                                                                       hidden_state, prev_c, attn)
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        prev_h = prev_h.to(device='cuda', dtype=torch.float64)
    
        A = x.mm(self.Wx.to('cuda')) + prev_h.mm(self.Wh.to('cuda')) + self.b.to('cuda')
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        x = x.to(device='cuda', dtype=torch.float64)
        prev_h = prev_h.to(device='cuda', dtype=torch.float64)
        attn = attn.to(device='cuda', dtype=torch.float64)
        
        A = x.mm(self.Wx.to('cuda')) + prev_h.mm(self.Wh.to('cuda')) + attn.mm(self.Wattn.to('cuda')) + self.b.to('cuda')
        a_i = torch.sigmoid(A[:, :H])
        a_f = torch.sigmoid(A[:, H:2*H])
        a_o = torch.sigmoid(A[:, 2*H:3*H])
        a_g = torch.tanh(A[:, 3*H:])
