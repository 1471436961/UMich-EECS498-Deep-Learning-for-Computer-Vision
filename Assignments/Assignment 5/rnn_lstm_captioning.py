...
def rnn_step_backward(dnext_h, cache):
    ...
    dnext_h_with_tanh = (1 - next_h * next_h) * dnext_h

    dx = dnext_h_with_tanh.mm(Wx.t())
    dprev_h = dnext_h_with_tanh.mm(Wh.t())
    dWx = x.t().mm(dnext_h_with_tanh)
    dWh = prev_h.t().mm(dnext_h_with_tanh)
    db = dnext_h_with_tanh.sum(dim=0)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = torch.zeros(N, T, H, dtype=torch.float64, device='cuda')
    cache = []
    prev_h = h0
...
def rnn_backward(dh, cache):
    ...
    dx = torch.zeros(N, T, D, dtype=torch.float64, device='cuda')
    dh0 = torch.zeros(N, H, dtype=torch.float64, device='cuda')
    dWx = torch.zeros(D, H, dtype=torch.float64, device='cuda')
    dWh = torch.zeros(H, H, dtype=torch.float64, device='cuda')
    db = torch.zeros(H, dtype=torch.float64, device='cuda')
    dprev_h = torch.zeros(N, H, dtype=torch.float64, device='cuda')

    for t in range(T-1, -1, -1):
        total_dout = dprev_h + dh[:, t, :]
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(total_dout, cache[t])
        dx[:, t, :] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    #     input,          # 模型原始输出（未经过Softmax，形状为 [N, C]）
    #     target,         # 真实标签（形状为 [N](类别索引（常用）) 或 [N, C](类别概率（如标签平滑后）)）
    #     weight=None,    # 各类别的权重（Tensor，可选）, 处理类别不平衡
    #     ignore_index=-100,  # 忽略的标签索引（默认-100）
    #     reduction='mean',   # 损失聚合方式：'mean'|'sum'|'none'
    #     label_smoothing=0.0  # 标签平滑系数（默认0，范围[0, 1)）, label_smoothing=0.1  # 将真实标签从1.0平滑为0.9，其余类别共享0.1/(C-1)
    # )
    # cross_entropy 不直接支持多标签分类（每个样本可能属于多个类别），此时应使用 torch.nn.BCEWithLogitsLoss
    # 性能优化建议
    #     启用CuDNN：确保使用GPU时启用加速：torch.backends.cudnn.enabled = True
    #     避免重复计算：如果同时需要损失和预测概率，直接使用 logits 计算：probs = F.softmax(logits, dim=1)  # 无需重复计算
    ################################NOTES#####################################
    ...
    ################################NOTES#####################################
    # 为什么用 reshape？
    #     将时间步和批次合并后，F.cross_entropy 会自动逐元素计算损失，无需手动循环。
    # reduction='sum' 的必要性
    #     先求和所有有效时间步的损失，再除以 N，等价于“按时间步求和，按批次平均”。 
    ################################NOTES#####################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        self.linear_img_to_hid = torch.nn.Linear(self.image_encoder.out_channels * 4 * 4, hidden_dim).to(device='cuda', dtype=torch.float64)
        self.word_embedding = WordEmbedding(vocab_size, embed_size=wordvec_dim).to(device='cuda', dtype=torch.float64)
        
        if self.cell_type == 'rnn':
            self.RNN = RNN(wordvec_dim, hidden_dim).to(device='cuda', dtype=torch.float64)
        elif self.cell_type == 'lstm':
            self.LSTM = LSTM(wordvec_dim, hidden_dim).to(device='cuda', dtype=torch.float64)
        else:
            self.conv_attention = torch.nn.Conv2d(self.image_encoder.out_channels, hidden_dim, kernel_size=1).to(device='cuda', dtype=torch.float64)
            self.AttentionLSTM = AttentionLSTM(wordvec_dim, hidden_dim).to(device='cuda', dtype=torch.float64)

        self.linear_hid_to_score = torch.nn.Linear(hidden_dim, vocab_size).to(device='cuda', dtype=torch.float64)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images, captions):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        image_features = self.image_encoder.forward(images).to(device='cuda', dtype=torch.float64)
        flatten_features = image_features.view(image_features.shape[0], -1)
        h0 = self.linear_img_to_hid.to(device='cuda', dtype=torch.float64)(flatten_features)

        word_embedding = self.word_embedding.forward(captions_in)
        
        if self.cell_type == 'rnn':
            hidden_states = self.RNN.forward(word_embedding, h0).to(device='cuda', dtype=torch.float64)
        elif self.cell_type == 'lstm':
            hidden_states = self.LSTM.forward(word_embedding, h0).to(device='cuda', dtype=torch.float64)
        else:
            attention_matrix = self.conv_attention(image_features)
            hidden_states = self.AttentionLSTM.forward(word_embedding, attention_matrix)
            
        scores = self.linear_hid_to_score.to(device='cuda', dtype=torch.float64)(hidden_states)
        loss = temporal_softmax_loss(scores, captions_out.to(device='cuda'), ignore_index=self._null)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def sample(self, images, max_length=15):
        ...
        image_features = self.image_encoder.forward(images).to(device='cuda', dtype=torch.float64)
        flatten_features = image_features.view(image_features.shape[0], -1)
        hidden_state = self.linear_img_to_hid(flatten_features)

        if self.cell_type == 'lstm':
            prev_c = torch.zeros(N, hidden_state.shape[1], device='cuda', dtype=torch.float64)
        elif self.cell_type == 'attn':
            attention_matrix = self.conv_attention(image_features)
            hidden_state = attention_matrix.mean(dim=(2, 3))
            prev_c = hidden_state

        word_embedding = self.word_embedding.forward([self._start for _ in range(N)])
 
        for i in range(max_length):
            if self.cell_type == 'rnn':
                hidden_state = self.RNN.step_forward(word_embedding, hidden_state)
            elif self.cell_type == 'lstm':
                hidden_state, prev_c = self.LSTM.step_forward(word_embedding, hidden_state, prev_c)
            else:
                attn, attn_weights = dot_product_attention(prev_h, images)
                attn_weights_all[:,t] = attn_weights
                hidden_state, prev_c = self.AttentionLSTM.step_forward(word_embedding, hidden_state, prev_c, attn)
            
            scores = self.linear_hid_to_scores(hidden_state)
            _, ind = scores.max(dim=1)
            word_embedding = self.word_embedding_forward(ind)
            captions[:, t] = ind
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        A = x.to(device='cuda', dtype=torch.float64).mm(self.Wx.to(device='cuda', dtype=torch.float64)) + prev_h.to(device='cuda', dtype=torch.float64).mm(self.Wh.to(device='cuda', dtype=torch.float64)) + self.b.to(device='cuda', dtype=torch.float64)
        a_i = torch.sigmoid(A[:, :H])
        a_f = torch.sigmoid(A[:, H:2*H])
        a_o = torch.sigmoid(A[:, 2*H:3*H])
        a_g = torch.tanh(A[:, 3*H:])

        next_c = a_f * prev_c + a_i * a_g
        next_h = a_o * torch.tanh(next_c)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        prev_h, prev_c = h0, c0
        hn = torch.zeros(N, T, H, device='cuda', dtype=torch.float64)
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    prev_h_temp = prev_h.reshape(N, 1, H)
    A_temp = A.reshape(N, H, -1)

    attn_weights_temp = prev_h_temp.bmm(A_temp).div(math.sqrt(H)).reshape(N, -1, 1).softmax(dim=1)
    attn = A_temp.bmm(attn_weights_temp).reshape(N, H)

    attn_weights = attn_weights_temp.reshape(N, 4, 4)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        A = x.to(device='cuda', dtype=torch.float64).mm(self.Wx.to(device='cuda', dtype=torch.float64)) + prev_h.to(device='cuda', dtype=torch.float64).mm(self.Wh.to(device='cuda', dtype=torch.float64)) + attn.to(device='cuda', dtype=torch.float64).mm(self.Wattn.to(device='cuda', dtype=torch.float64)) + self.b.to(device='cuda', dtype=torch.float64)
        a_i = torch.sigmoid(A[:, :H])
        a_f = torch.sigmoid(A[:, H:2*H])
        a_o = torch.sigmoid(A[:, 2*H:3*H])
        a_g = torch.tanh(A[:, 3*H:])
