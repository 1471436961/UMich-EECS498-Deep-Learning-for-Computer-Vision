...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for channel in stem_channels:
            stem_cls.append(torch.nn.Conv2d(in_channels, channel, 
                                            kernel_size=3, padding=1))
            stem_cls.append(torch.nn.ReLU())

            stem_box.append(torch.nn.Conv2d(in_channels, channel, 
                                            kernel_size=3, padding=1))
        ...
        # Replace "pass" statement with your code
        self.pred_cls = torch.nn.Conv2d(in_channels, num_classes, 
                                        kernel_size=3, padding=1)
        self.pred_box = torch.nn.Conv2d(in_channels, 4, 
                                        kernel_size=3, padding=1)
        self.pred_ctr = torch.nn.Conv2d(in_channels, 1, 
                                        kernel_size=3, padding=1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feat in feats_per_fpn_level.items():
            ...
            box_delta = self.pred_box(box_feat)         
            ctr_logit = self.pred_ctr(box_feat)          
    
            B, num_classes, H, W = cls_logit.shape
            class_logits[level_name] = cls_logit.permute(0, 2, 3, 1).reshape(B, -1, num_classes)
            boxreg_deltas[level_name] = box_delta.permute(0, 2, 3, 1).reshape(B, -1, 4)
            centerness_logits[level_name] = ctr_logit.permute(0, 2, 3, 1).reshape(B, -1, 1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_match_locations_to_gt(
    ...
    for level_name, centers in locations_per_fpn_level.items():
        ...
        ##############################NOTES###################################
        # torch.unbind()用于沿指定维度解绑张量，它将张量拆分为多个子张量序列。
        # 常与 stack 互为逆操作（如 stack(unbind(x)) == x）。
        # torch.unbind(input, dim=0) -> tuple
        # 参数：input：要解绑的张量。 dim：指定解绑的维度（默认 dim=0）。
        # 返回：包含切片后的子张量的元组（数量等于 input 在 dim 维度的大小）
        #             与 torch.split() 和 torch.chunk() 的区别
        #   函数         行为	              返回类型	       典型用途
        # unbind()	按维度完全拆分（每片大小=1）	元组	网格坐标处理、逐元素操作
        # split()	按指定大小拆分	            元组	变长数据分块（如文本序列）
        # chunk()	按指定数量均分	            元组	均等分块（如并行计算）
        ##############################NOTES###################################
