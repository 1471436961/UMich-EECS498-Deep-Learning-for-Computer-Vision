...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        # pad(tuple):一个元组，指定张量每个维度的填充大小。元组的长度必须是偶数，
        # 格式为(pad_left, pad_right, pad_top, pad_bottom, ...)，依次对应每个维度。
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.params['W1'] = weight_scale * torch.randn(num_filters, input_dims[0], 
        ...
        self.params['W2'] = weight_scale * torch.randn(num_conv_relu_pool,  hidden_dim, 
                                                       dtype=self.dtype, device=device)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        if weight_scale == 'kaiming':
            num_filters.append(input_dims[0]) # 编码技巧，使得网络第一层参数的初始化不用单独处理
            for i in range(1, self.num_layers):
                self.params[f'W{i}'] = kaiming_initializer(num_filters[i - 1], num_filters[i - 2], K=3, 
            ...
            num_filters.pop() # 还原，以便无需更改后续代码
            
            num_full = num_filters[-1] * input_dims[1] * input_dims[2] // ((2 ** len(self.max_pools)) ** 2)
            self.params[f'W{self.num_layers}'] = kaiming_initializer(num_full, num_classes, K=None, relu=False, 
                                                                     dtype=self.dtype, device=device)
            self.params[f'b{self.num_layers}'] = torch.zeros(num_classes, dtype=self.dtype, device=device)
        else:
            num_filters.append(input_dims[0]) # 编码技巧，使得网络第一层参数的初始化不用单独处理
            for i in range(1, self.num_layers):
                self.params[f'W{i}'] = weight_scale * torch.randn(num_filters[i - 1], num_filters[i - 2],
                                                                 3, 3, dtype=self.dtype, device=device)
                self.params[f'b{i}'] = torch.zeros(num_filters[i - 1], dtype=self.dtype, device=device)
            ...
            num_full = num_filters[-1] * input_dims[1] * input_dims[2] // ((2 ** len(self.max_pools)) ** 2)
            self.params[f'W{self.num_layers}'] = weight_scale * torch.randn(num_full, num_classes, 
                                                                            dtype=self.dtype, device=device)
            self.params[f'b{self.num_layers}'] = torch.zeros(num_classes, dtype=self.dtype, device=device)
        ################################################################
        #                      END OF YOUR CODE                        #
    ...
    def loss(self, X, y=None):
        ...
        if self.batchnorm:
            for i in range(1, self.num_layers):
                if (i - 1) in self.max_pools:
                    out, all_cache[f'{i}'] = Conv_BatchNorm_ReLU_Pool.forward(out, self.params[f'W{i}'], self.params[f'b{i}'],
                                                                              self.params[f'gamma{i}'], self.params[f'beta{i}'],
                                                                              conv_param, self.bn_params[i - 1], pool_param)
                else:
                    out, norm_cache[f'{i}'] = Conv_BatchNorm_ReLU.forward(out, self.params[f'W{i}'], self.params[f'b{i}'],
                                                                         self.params[f'gamma{i}'], self.params[f'beta{i}'],
                                                                         conv_param, self.bn_params[i - 1])
        else:
            for i in range(1, self.num_layers):
                if (i - 1) in self.max_pools:
                    out, pool_cache[f'{i}'] = Conv_ReLU_Pool.forward(out, self.params[f'W{i}'], self.params[f'b{i}'],
                                                                    conv_param, pool_param)
                else:
                    out, conv_cache[f'{i}'] = Conv_ReLU.forward(out, self.params[f'W{i}'], self.params[f'b{i}'], conv_param)
        ...
        if self.batchnorm:
            ...
            for i in range(self.num_layers - 1, 0, -1):
                if (i - 1) in self.max_pools:
                    temp = Conv_BatchNorm_ReLU_Pool.backward(dout, all_cache[f'{i}'])
                    dout, grads[f'W{i}'], grads[f'b{i}'], grads[f'gamma{i}'], grads[f'beta{i}'] = temp
                else:
                    temp = Conv_BatchNorm_ReLU.backward(dout, norm_cache[f'{i}'])
                    dout, grads[f'W{i}'], grads[f'b{i}'], grads[f'gamma{i}'], grads[f'beta{i}'] = temp
        else:
            for i in range(self.num_layers - 1, 0, -1):
                if (i - 1) in self.max_pools:
                    dout, grads[f'W{i}'], grads[f'b{i}'] = Conv_ReLU_Pool.backward(dout, pool_cache[f'{i}'])
                else:
                    dout, grads[f'W{i}'], grads[f'b{i}'] = Conv_ReLU.backward(dout, conv_cache[f'{i}'])
...
class FastConv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        #     stride (int or tuple, 可选): 卷积操作的步幅。
        #     padding (int or tuple, 可选): 输入数据的填充大小。0表示不填充。
        #     dilation (int or tuple, 可选): 空洞卷积的膨胀率。1表示普通卷积。
        #     groups (int, 可选): 分组卷积的组数。默认值为 1（普通卷积）。
        #         groups > 1则输入和输出通道会被分成groups组，每组独立进行卷积。
        #     bias (bool, 可选): 是否使用偏置项。默认值为 True。
        #     padding_mode (str, 可选): 填充模式。默认值为 'zeros'（零填充）。
        #         其他可选模式包括 'reflect'、'replicate' 和 'circular'。
        ##############################NOTES###############################
        layer = torch.nn.Conv2d(C, F, (HH, WW), stride=stride, padding=pad)
        ##############################NOTES###############################
        # torch.nn.Parameter(data, requires_grad=True)将张量标记为模型参数。
        # 是 torch.Tensor的子类，作用是告诉 PyTorch该张量需要在训练过程中更新。
        # 自动注册为模型参数：当torch.nn.Parameter被赋值给torch.nn.Module的属性时，
        # 会自动注册为模型参数。意味着会被包含在model.parameters()中，在训练过程中被优化器更新。
        # 支持梯度计算：torch.nn.Parameter 的 requires_grad 默认为 True，因此会自动计算梯度。
        # 与普通张量的区别：与普通张量不同，torch.nn.Parameter 会被 PyTorch 自动识别为模型参数
        ##############################NOTES###############################
        layer.weight = torch.nn.Parameter(w)
        layer.bias = torch.nn.Parameter(b)
        ##############################NOTES###############################
        # detach(), PyTorch中的一个方法，用于从计算图中分离张量。返回一个新的张量，
        # 该张量与原始张量共享数据，但不会参与梯度计算。会切断反向传播时梯度流向该张量的路径。
        # 使用场景
        #     阻止梯度计算：当你希望某些张量不参与反向传播时，可以使用 detach()。
        #            例如，在GAN中，生成器的输出在训练判别器时不需要梯度。
        #     获取张量的值：当你需要获取张量的值（而不是计算图的一部分）时，可以使用 detach()。
        #            例如，将张量从 GPU 移动到 CPU 或转换为 NumPy 数组时。
        #     避免内存泄漏：在某些情况下，使用detach()可以避免不必要的计算图保存，从而减少内存占用。
        # 注意事项
        #     共享数据：detach()返回的张量与原始张量共享数据，修改其中一个张量会影响另一个张量。
        #            如果需要完全独立的张量，可以使用detach().clone()。
        #     与 with torch.no_grad()的区别：detach()是张量级别的操作，仅对特定张量生效。
        #          with torch.no_grad()是上下文管理器，会禁用整个代码块中的梯度计算。
        #     与 requires_grad=False的区别：detach()返回的张量仍然可以手动设置 requires_grad=True。
        #         直接设置 requires_grad=False 会完全禁用梯度计算，且无法重新启用。
        ##############################NOTES###############################
        tx = x.detach()
    ...
    def backward(dout, cache):
        try:
            ...
            # 自动求导的核心方法。通过反向传播计算张量的梯度，并将梯度存储在张量的.grad属性中。
            # 参数
            #     gradient (Tensor or None, 可选): 用于链式求导的梯度。
            #         通常用于计算非标量张量的梯度。默认值为 None，表示标量张量的梯度为 1。
            #     retain_graph (bool, 可选): 是否保留计算图。为 True则计算图不会被释放，
            #         可以多次调用 backward()。为 False则计算图会在调用backward()后被释放。
            #     create_graph (bool, 可选): 是否创建计算图以支持高阶导数。
            #         默认值为 False。如果为 True，则可以计算二阶或更高阶的导数。
            # 使用场景
            ...
            # 注意事项
            #     梯度累加：每次调用 backward() 时，梯度会累加到 .grad 属性中。
            #         如果需要清零梯度，可以调用 x.grad.zero_()。
            #     释放计算图：默认情况下，调用 backward() 后会释放计算图。
            #         如果需要多次调用 backward()，需要设置 retain_graph=True。
...
class FastMaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        #     stride (int or tuple, 可选): 池化操作的步幅。默认值为 kernel_size。
        #     padding (int or tuple, 可选): 输入数据的填充大小。0表示不填充。
        #     dilation (int or tuple, 可选): 空洞池化的膨胀率。1表示普通池化。
        #     return_indices (bool, 可选): 是否返回最大值的位置索引。
        #         为 True则返回一个元组 (output, indices)，indices是最大值的位置索引。
        #     ceil_mode (bool, 可选): 是否使用 ceil 模式计算输出大小。
        #         如果为 True，则使用 ceil 函数计算输出大小；否则使用 floor 函数
        ##############################NOTES###############################
