...
class Linear(object):
    '''
    @staticmethod 是 Python 中的一个装饰器，用于定义静态方法。静态方法是类中的一种特殊方法，它不需要访问类的实例（self）或类本身（cls），
    因此它既不能修改实例状态，也不能修改类状态。静态方法通常用于实现与类相关但不需要访问类或实例数据的工具函数。
    特点：
        不需要 self 或 cls 参数：静态方法不需要传递类的实例（self）或类本身（cls）作为参数。
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        * 解包操作符	        Python 3.5+ 的简洁拼接方式	  语法简洁，适合现代 Python 代码
        '''
        net_dims = [input_dim] + hidden_dims + [num_classes]
        for i in range(self.num_layers):
            ind = i + 1
            self.params[f'W{ind}'] = weight_scale * torch.randn(net_dims[i], net_dims[ind], dtype=self.dtype, device=device)
            self.params[f'b{ind}'] = torch.zeros(net_dims[ind], dtype=self.dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        scores, cache['1'] = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        
        if self.use_dropout and self.dropout_param['p'] != 0:
            scores, cache_drop['1'] = Dropout.forward(scores, self.dropout_param)
            
        for i in range(self.num_layers - 2):
            ind = i + 2
            scores, cache[f'{ind}'] = Linear_ReLU.forward(scores, self.params[f'W{ind}'], self.params[f'b{ind}'])
            
            if self.use_dropout and self.dropout_param['p'] != 0:
                scores, cache_drop[f'{ind}'] = Dropout.forward(scores, self.dropout_param) 
                
        scores, cache[f'{self.num_layers}'] = Linear.forward(scores, self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'])
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        dh, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(dscores, cache[f'{self.num_layers}'])
        
        for i in range(self.num_layers - 2):
            ind = self.num_layers - i - 1
            if self.use_dropout and self.dropout_param['p'] != 0:
                dh = Dropout.backward(dh, cache_drop[f'{ind}'])
                
            dh, grads[f'W{ind}'], grads[f'b{ind}'] = Linear_ReLU.backward(dh, cache[f'{ind}'])

        if self.use_dropout and self.dropout_param['p'] != 0:
            dh = Dropout.backward(dh, cache_drop['1'])
            
        dx, grads['W1'], grads['b1'] = Linear_ReLU.backward(dh, cache['1'])
